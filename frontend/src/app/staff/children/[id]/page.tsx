...
export default function ChildPage({ params }: Props) {
    ...
    return (
        <>
            ...
            <Form title="">
                <Field onChange={(e) => handleChange("first_name", e.target.value)} title={"Name: "} value={child?.first_name} type={'text'} name={'firstName'} disabled={!canEdit} hidden={false}/>
                <Field onChange={(e) => handleChange("last_name", e.target.value)} title={"Lastname:"} value={child?.last_name} type={'text'} name={'lastName'} disabled={!canEdit} hidden={false}/>
